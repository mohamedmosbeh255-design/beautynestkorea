...
const markdownComponents = {
  ...
  blockquote: ({ children }: { children?: ReactNode }) => (
    <blockquote className="rounded-r-2xl border-l-4 border-amber-400 bg-amber-50 px-5 py-4 text-[0.95rem] leading-relaxed text-amber-900 [&>p]:my-2">
      {children}
    </blockquote>
  ),
  details: ({ children }: { children?: ReactNode }) => (
    <details className="glass group rounded-2xl px-5 py-4 [&>summary]:cursor-pointer">
      {children}
    </details>
  ),
  summary: ({ children }: { children?: ReactNode }) => (
    <summary className="text-[1rem] font-semibold text-ink marker:text-sage-600">
      {children}
    </summary>
  ),
  ...
  h2: ({ children }: { children?: ReactNode }) => (
    <h2 id={slugifyHeading(children)} className="font-serif-display scroll-mt-24 pt-4 text-2xl font-bold tracking-tight text-ink">
      {children}
    </h2>
  ),
  h3: ({ children }: { children?: ReactNode }) => (
    <h3 id={slugifyHeading(children)} className="font-serif-display scroll-mt-24 pt-2 text-xl font-bold tracking-tight text-ink">
      {children}
    </h3>
  ),
