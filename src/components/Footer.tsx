...
const socials = [
  ...
  { href: "https://benable.com/BeautyNest2026", label: "Benable", Icon: ShoppingBag },
  { href: "https://c8ke.me/beautynestkorea", label: "Short links", Icon: LinkIcon },
  { href: "https://sites.google.com/view/beautynestskincare/kbeauty-serums", label: "Google Sites", Icon: FileText },
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 md:grid-cols-5">
        <div className="md:col-span-2">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {/* Social placeholders (B6): profiles not claimed yet — non-link
                spans so they can never read as dead links to users or crawlers. */}
            {[
              { label: "TikTok (coming soon)", Icon: Music2 },
              { label: "YouTube (coming soon)", Icon: YoutubeIcon },
            ].map(({ label, Icon }) => (
              <span
                key={label}
                title={label}
                aria-label={label}
                className="flex h-9 w-9 cursor-default items-center justify-center rounded-full bg-sage-50/50 text-sage-700/40"
              >
                <Icon className="h-4 w-4" />
              </span>
            ))}
